...
- **A person may always overrule the matcher.** Faces that fail a quality gate are stored with `faces.rejected` naming which one and are never clustered, so somebody can identify them by eye; `library.assign_face` moves a face and, when the face was usable, adds its embedding as a look so the correction teaches the matcher, while a rejected face never does. `library.tag_item` records somebody in a photograph with no detected face at all (a `faces` row with no bbox and no embedding). Anything human-assigned carries `faces.manual` and clustering never revisits it.
- **Runtime config beats the environment.** `library.get_config`/`set_config` back an admin override for the captioning provider, the captioning model and the codec checkpoint; `vision` reads them on every call, so a model change applies to the next photograph rather than the next deploy. Clearing an override restores the env default. `/api/admin/*` needs `JSONCAM_ADMIN_KEY` and refuses when it is unset; it can change settings and report totals, and deliberately cannot read or list a library.
